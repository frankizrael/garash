...
class woofiltersControllerWpf extends controllerWpf {
	...
	public function getFilterTerms($args, $recount = true){
        ...
        if($filterLoop->have_posts()) {
        	$ignore = array('product_visibility','product_type');
        	$terms = get_terms(array('object_ids' => $filterLoop->posts, 'hide_empty' => 1, 'hierarchical' => 1));
        	foreach ($terms as $term) {
        		$termName = $term->taxonomy;
        		if(in_array($termName, $ignore)) continue;

        		$isTag = $termName == 'product_tag';
        		$name = ($isTag ? 'tag' : urldecode(str_replace('pa_', '', $termName)));

        		$termId = $term->term_id;
        		if(isset($existTerms[$name])) array_push($existTerms[$name], $termId);
        		else $existTerms[$name] = array($termId);
        		if($recount) {
        			$isCat = $termName == 'product_cat';
        			$name = ($isCat ? 'filter_cat' : urldecode($termName));
        			if(!isset($countProducts[$name])) $countProducts[$name] = array();
        			//$countProducts[$name][$isTag || $isCat ? $termId : $term->slug] = $term->count;
        			$countProducts[$name][$termId] = $term->count;    			
        		}
        	}
        }
        
        return array('exists' => $existTerms,  'count' => $countProducts);
    }
	...
	public function filtersFrontend(){
		...
		$terms = $this->getFilterTerms($args, $recount);

		if($recount) {
			$productsHtml .= '<script type="text/javascript">wpfChangeFiltersCount('.json_encode($terms['count']).');</script>';
        }
		...
		$productCounter = isset($terms['count']['filter_cat']) ? $terms['count']['filter_cat'] : array();
		$categoryIn = array();
